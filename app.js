  ...
  },
  {
    ...
    "items": [
      ...
      },
      {
        ...
        "aws": "Enables quick rollback and root cause when changes hit staging/production."
      }
    ],
...
// State
let mode = localStorage.getItem("cli.mode") || "guided"; // "guided" | "free"
let track = localStorage.getItem("cli.track") || "git";
let day = 1; // per mode (guided uses global index, free uses per-track index)
let checks = JSON.parse(localStorage.getItem("cli.checks") || "{}"); // keys: `${track}-${day}-i`
...
function setStoredDay(value) {
  if (mode === "guided") localStorage.setItem("cli.day.guided", String(value));
  else localStorage.setItem(`cli.day.${track}`, String(value));
}

// Guided mapping (global index -> track/day)
function mapGuidedIndex(idx) {
  ...
  return { track: "linux", day: LENGTHS.linux }; // clamp
}
...
function renderProgress(){
  ...
  completedEl.textContent = String(completed) + " / " + String(denom);
...
function currentDayData(){
  if (mode === "guided") {
    const map = mapGuidedIndex(day);
    return { map, data: TRACKS[map.track][map.day-1] };
  } else {
...
function renderDay(){
  ...
  const d = Math.max(1, Math.min(len, day));
  day = d;
  setStoredDay(day);

  // Lock/unlock track select visibility
  elTrack.disabled = (mode === "guided");
  elTrack.style.opacity = (mode === "guided") ? "0.6" : "1";

  const { map, data } = currentDayData();
  const titlePrefix = (mode === "guided") ? `Day ${d} of ${TOTAL_DAYS} — ${map.track.toUpperCase()}` : `Day ${d}`;
  ...
  // Resume bullet (auto) – combine day items
  const b = data.resume || "";
  resumeBullet.textContent = b;
  ...
  document.querySelectorAll(".check").forEach(el => {
    el.addEventListener("click", ()=> {
      ...
      if (both) {
        if (lastCompletedDay === (mode==='guided'? day-1 : day-1)) streak += 1;
        if (lastCompletedDay < (mode==='guided'? day : day)) lastCompletedDay = (mode==='guided'? day : day);
  ...
  copyBullet.onclick = async () => {
    try {
      ...
      copyBullet.textContent = "Copied!";
      setTimeout(()=> copyBullet.textContent = "Copy bullet", 1200);
...
enableAlerts.addEventListener("click", async ()=>{
  try{ const perm = await Notification.requestPermission();
    if (perm === "granted"){
      new Notification("CLI Mastery", { body: "Daily reminders enabled." });
      enableAlerts.textContent = "Alerts Enabled";
    }
  }catch(e){ console.warn(e); }
});
...
installBtn.addEventListener('click', async ()=>{
  ...
  deferredPrompt = null;
  installBtn.disabled = true; installBtn.textContent = 'Installed';
});
offlineBtn.addEventListener('click', async ()=>{
  try{ const reg = await navigator.serviceWorker.register('./service-worker.js');
    if(reg){ swDot.classList.remove('offline'); swDot.classList.add('online'); swText.textContent = 'Offline enabled'; }
